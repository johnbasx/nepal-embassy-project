...
const Filter = ({ dataList, filterValues }: FilterProps<string>) => {
  const [filteredData, setFilteredData] = useState(dataList);
  const [selected, setSelected] = useState(filterValues[0]);

  const people = [
    { name: 'All', dataToFilter: 'ansds' },
    { name: 'Verified', dataToFilter: 'ansds' },
    { name: 'Pending', dataToFilter: 'ansds' },
    { name: 'Rejected', dataToFilter: 'ansds' },
  ];
  ...
  useEffect(() => {
    filterHandler(selected.name, selected.name);
  }, [selected]);
